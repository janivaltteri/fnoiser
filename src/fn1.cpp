...
// mean 0.0 sd 1.0
RcppExport SEXP fn1(SEXP time, SEXP gamma)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_real_distribution<double> dist(1.0, 10.0);
  const double pii = 3.14159265359;
  double t, g, ss, spec, vaihe, freq;
...
// a function for a dual fnoise run with given correlation
// mean 0.0 sd 1.0
RcppExport SEXP fn2(SEXP time, SEXP gamma, SEXP corr)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_real_distribution<double> dist(1.0, 10.0);
  const double pii = 3.14159265359;
  double t, g, rho, ss, spec, vaihe, freq;
  Rcpp::NumericVector timev(time);
  Rcpp::NumericVector gammav(gamma);
  Rcpp::NumericVector corrv(corr);
  t = timev[0];
  g = gammav[0];
  rho = corrv[0];
  ss = t/2;
  //change to std::vector<std::vector<double>>
  std::vector<double> w, x;
  w.resize(t);
  x.resize(t);
  for(int i = 0; i < t; ++i){
